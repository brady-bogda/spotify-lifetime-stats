...
csv_filepaths = glob.glob(os.path.join(sys.argv[1], "endsong_*.json"))

print("Number of files: ", len(csv_filepaths))
...
# lifetime_data = lifetime_data.loc[lifetime_data["year"] == 2024]
# lifetime_data = lifetime_data.loc[lifetime_data["month"] < 11]

# drop columns
cols_to_drop = {
    "incognito_mode", "time", "offline", "offline_timestamp", "platform",
    "shuffle", "skipped", "reason_end", "reason_start", "episode_show_name",
    "spotify_episode_uri", "episode_name", "spotify_track_uri"}
# these columns are no longer in the json (as of Dec 2024)
# old_cols_to_drop = {"ip_addr_decrypted", "user_agent_decrypted", "username"}
lifetime_data.drop(columns=cols_to_drop, inplace=True)


top_artists = lifetime_data.groupby(
    ["master_metadata_album_artist_name"])["ms_played"].sum().reset_index(
    name="ms_played").sort_values(
...
top_artists.rename(
    columns={"master_metadata_album_artist_name": "Artist name",
             "ms_played": "Minutes"},
    inplace=True)
top_artists["Percentage of Minutes"] = round(
    top_artists["Minutes"]/top_artists["Minutes"].sum()*100, 2)


top_songs = lifetime_data.groupby(
    ["master_metadata_track_name", "master_metadata_album_artist_name"])[
    "ms_played"].agg(
    ms_played='sum', Number_of_listens='count').reset_index().sort_values(
    by="Number_of_listens", ascending=False).reset_index(
    drop=True)
...
top_songs.rename(
    columns={"master_metadata_track_name": "Song name",
             "master_metadata_album_artist_name": "Artist name",
             "ms_played": "Minutes",
                          "Number_of_listens": "Number of listens"},
    inplace=True)


top_albums = lifetime_data.groupby(["master_metadata_album_album_name",
                                    "master_metadata_album_artist_name"])[
    "ms_played"].sum().reset_index(name="ms_played").sort_values(
    by="ms_played", ascending=False).reset_index(drop=True)
...
top_albums.rename(
    columns={"master_metadata_album_album_name": "Album name",
             "master_metadata_album_artist_name": "Artist name",
             "ms_played": "Minutes"},
    inplace=True)


top_songs_winter = lifetime_data.loc[(lifetime_data["month"] == 12) | (
    (lifetime_data["month"] == 11) & (lifetime_data["day"] > 25))]
top_songs_winter = top_songs_winter.groupby(
    ["master_metadata_track_name", "master_metadata_album_artist_name"])[
...
top_songs_winter.rename(
    columns={"master_metadata_track_name": "Song name",
             "master_metadata_album_artist_name": "Artist name",
             "ms_played": "Minutes"},
    inplace=True)

top_songs_summer = lifetime_data.loc[(
    lifetime_data["month"] > 5) & (lifetime_data["month"] < 9)]
top_songs_summer = top_songs_summer.groupby(
    ["master_metadata_track_name", "master_metadata_album_artist_name"])[
    "ms_played"].sum().reset_index(
    name="ms_played").sort_values(
    by="ms_played", ascending=False).reset_index(
...
print(top_artists.head(num_top).to_csv(sep='|'))
print("\n\nTOP ", num_top, " SONGS")
print(top_songs.head(num_top).to_csv(sep='|'))
print("\n\nTOP ", num_top, " ALBUMS")
print(top_albums.head(num_top).to_csv(sep='|'))
